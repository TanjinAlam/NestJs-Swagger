...
export class AuthService {
     ...
     * @return  {User}
     */
    public async userLogin(loginDTO: LoginDTO): Promise<User> {
    ...
    public async login(loginDTO: LoginDTO): Promise<User> {
        const user = await this.findAdmin(loginDTO.email)
        if (!user || user.userType != 'Admin') {
            throw new NotFoundException('Admin is not registered')
        }
        if (!this.isPasswordValid(loginDTO.password, user.password)) {
            throw new BadRequestException('Invalid password')
        }
